...
$$FWER = P \left( \bigcup \limits_{i=1}^m \{\text{reject } H_{0,i} | H_{0,i} \text{ is true}\} \right)$$

.question[
Suppose all $m$ tests are independent. For each test, we reject if the corresponding p-value $p_i < \alpha$. What is the FWER?
]
]
...
.question[
If all $m$ hypotheses are independent, at what threshold $\alpha^*$ should we reject each test, such FWER = $\alpha$? 
]
]
